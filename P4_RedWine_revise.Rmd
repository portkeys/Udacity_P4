...
From the above heat map, we notice the association patterns among independent variables:
...
before <- polyserial(dta$chlorides,dta$level)
after <- polyserial(log10(dta$chlorides), dta$level)
before
after
```
...
acidgroup <- dta[,1:4]
acidgroup$residual.sugar <- log10(acidgroup$residual.sugar)
pairs.panels(acidgroup, bg=c("blue","grey","red")[dta$level],pch=21,lm=TRUE)
```
...
dat$log_residual.sugar <- log10(dat$residual.sugar)
dat$log_chlorides <- log10(dat$chlorides)

dat$residual.sugar <- NULL
str(dat)
