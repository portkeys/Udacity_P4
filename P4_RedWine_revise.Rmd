...
We used quality to check correlation for simplicity reason., since our ultimate goal is to predict variable *level*, let's check how 11 physicochemical attributes interacting with our target response variable *level* by boxplots.
...
```{r}
ggplot(dta, aes(x=citric.acid, y= pH, colour=level)) +
  facet_wrap(~ level) +
...
```{r}
ggplot(dta, aes(x=fixed.acidity, y= pH, colour=level)) +
  facet_wrap(~ level) +
  geom_point(alpha=0.3, position="jitter") +
  geom_boxplot(alpha=0, colour="black")
```
...
```{r}
ggplot(dta, aes(x=fixed.acidity, y=density)) +
